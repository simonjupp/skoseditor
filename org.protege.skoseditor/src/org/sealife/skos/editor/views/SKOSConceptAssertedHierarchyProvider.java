...
public class SKOSConceptAssertedHierarchyProvider extends AbstractSKOSHierarchyProvider implements OWLOntologyChangeListener {
    ...
    protected Set<OWLObjectProperty> loadBroaderProps() {
        return Collections.singleton(getManager().getOWLDataFactory().getOWLObjectProperty(SKOSVocabulary.BROADER.getIRI()));
    }

    protected Set<OWLObjectProperty> loadNarrowerProps() {
        return Collections.singleton(getManager().getOWLDataFactory().getOWLObjectProperty(SKOSVocabulary.NARROWER.getIRI()));
    }
...
//        setFireEvents(false);
        ...
        skosConcept = getManager().getOWLDataFactory().getOWLClass(SKOSVocabulary.CONCEPT.getIRI());
        ...
        for (OWLOntology ont : ontologies) {
            ...
            for (OWLClassAssertionAxiom axiom : ont.getAxioms(AxiomType.CLASS_ASSERTION)) {
                if (axiom.getClassExpression().equals(skosConcept)) {
                    conceptsToView.add(axiom.getIndividual().asOWLNamedIndividual());
                }
            }
...
//        setFireEvents(true);
    ...
    public void ontologiesChanged(List<? extends OWLOntologyChange> changes) throws OWLException {
        Set<OWLNamedIndividual> affectedIndividuals = null;
                ...
                if(change.getAxiom() instanceof OWLObjectPropertyAssertionAxiom || change.getAxiom() instanceof OWLClassAssertionAxiom) {
                    ...
                    if(ontologies.contains(change.getOntology())) {
                        ...
                        if (change.getAxiom() instanceof OWLObjectPropertyAssertionAxiom) {
                            affectedIndividuals.add(((OWLObjectPropertyAssertionAxiom) change.getAxiom()).getSubject().asOWLNamedIndividual());
                            affectedIndividuals.add(((OWLObjectPropertyAssertionAxiom) change.getAxiom()).getObject().asOWLNamedIndividual());
                        }
                        else if(change.getAxiom() instanceof OWLClassAssertionAxiom) {
                            affectedIndividuals.add(((OWLClassAssertionAxiom) change.getAxiom()).getIndividual().asOWLNamedIndividual());
                        }
                    }
                }
            }
        ...
        if (affectedIndividuals != null) {
            ...
            for(OWLNamedIndividual ind : affectedIndividuals) {
                fireNodeChanged(ind);
            }
        }
    }
