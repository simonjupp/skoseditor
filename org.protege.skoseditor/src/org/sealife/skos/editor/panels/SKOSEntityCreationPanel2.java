...
//        return owlEditorKit.getOWLModelManager().getActiveOntology().getURI();
        OWLNamedIndividual ind = getSelectedSchema();
        IRI iri = ind.getIRI();
        //System.err.println("host: " + base.toString());
        return URI.create("http://" + iri.toURI().getHost() + iri.toURI().getPath());
    }
    ...
    public OWLNamedIndividual getCurrentConceptScheme () {

        OWLOntology onto = owlEditorKit.getModelManager().getActiveOntology();
        Set<OWLClassAssertionAxiom> axioms = onto.getClassAssertionAxioms(owlEditorKit.getModelManager().getOWLDataFactory().getOWLClass(SKOSVocabulary.CONCEPTSCHEME.getIRI()));
        Iterator it = axioms.iterator();
        OWLClassAssertionAxiom ax = (OWLClassAssertionAxiom) it.next();
        return ax.getIndividual().asOWLNamedIndividual();

    }

    public OWLNamedIndividual getSelectedSchema () {
