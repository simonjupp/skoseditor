...
public class SKOSConceptAssertedHierarchyViewComponent extends AbstractHierarchyViewComponent implements Deleteable, CreateNewTarget {
    ...
    public void initialiseIndividualsView() throws Exception {
        ...
        getTree().setDragAndDropHandler(new OWLTreeDragAndDropHandler<OWLNamedIndividual>() {
            ...
            public void move(OWLNamedIndividual child, OWLNamedIndividual fromParent, OWLNamedIndividual toParent) {
                List<OWLOntologyChange> changes = new ArrayList<OWLOntologyChange>();
                changes.add(new AddAxiom(getOWLModelManager().getActiveOntology(),
                                         getOWLModelManager().getOWLDataFactory().getOWLObjectPropertyAssertionAxiom(getOWLModelManager().getOWLDataFactory().getOWLObjectProperty(SKOSVocabulary.BROADER.getIRI()), child,
                                                    toParent)));
                changes.add(new RemoveAxiom(getOWLModelManager().getActiveOntology(),
                                            getOWLModelManager().getOWLDataFactory().getOWLObjectPropertyAssertionAxiom(getOWLModelManager().getOWLDataFactory().getOWLObjectProperty(SKOSVocabulary.BROADER.getIRI()),
                                                                                                         child, fromParent)));
                getOWLModelManager().applyChanges(changes);
            }

            public void add(OWLNamedIndividual child, OWLNamedIndividual parent) {
                List<OWLOntologyChange> changes = new ArrayList<OWLOntologyChange>();
                changes.add(new AddAxiom(getOWLModelManager().getActiveOntology(),
                                         getOWLModelManager().getOWLDataFactory().getOWLObjectPropertyAssertionAxiom(getOWLModelManager().getOWLDataFactory().getOWLObjectProperty(SKOSVocabulary.BROADER.getIRI()),
                                                    child, parent)));
...
//        };
        ...
        addAction(new DeleteIndividualAction(getOWLEditorKit(), new OWLEntitySetProvider<OWLNamedIndividual>() {
                                                                          public Set getEntities() {
                                                                              return new HashSet<OWLNamedIndividual>(getTree().getSelectedOWLObjects());
                                                                          }
                                                                      }), "E", "A");
    ...
    public void addRelatedConcept() {
        ...
        //OWLEntityCreationSet<OWLNamedIndividual> set = getOWLWorkspace().createOWLNamedIndividual();
        ...
        if (ret == JOptionPane.OK_OPTION) {
            OWLNamedIndividual selectedInd = getSelectedOWLIndividual();
            if (selectedInd == null) {
                return;
            }
            OWLObjectPropertyIndividualPair pair = editor.getEditedObject();

            OWLAxiom ax = getOWLModelManager().getOWLDataFactory().getOWLObjectPropertyAssertionAxiom(pair.getProperty(), selectedInd, pair.getIndividual());
    ...
    private void addRelatedDataProperty() {
        ...
        if (ret == JOptionPane.OK_OPTION) {
            ...
            OWLAxiom ax = getOWLModelManager().getOWLDataFactory().getOWLDataPropertyAssertionAxiom(pair.getProperty(),selectedInd, pair.getConstant());
            this.getOWLModelManager().applyChange(new AddAxiom(getOWLModelManager().getActiveOntology(), ax));
        }
        editor.dispose();
    }
    ...
    public void addNarrowerConcept () {
        ...
        OWLNamedIndividual relatedInd = SKOSConceptSelectorPanel.showDialog(getOWLEditorKit());

        OWLNamedIndividual selected = getSelectedOWLIndividual();
        ...
        // type it as a SKOS concept
        SKOSConcept concept = skosFactory.getSKOSConcept(relatedInd.getIRI().toURI());
        SKOSConcept selectedConcept = skosFactory.getSKOSConcept(selected.getIRI().toURI());
